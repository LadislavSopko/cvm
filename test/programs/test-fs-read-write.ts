...
function main() {
  ...
  const notFound = fs.readFile("./does-not-exist.txt");
  ...
  const jsonResult = fs.writeFile("./data.json", JSON.stringify(data));
  console.log("JSON write result: " + jsonResult);
  
  const jsonRead = fs.readFile("./data.json");
  ...
  const overwriteResult = fs.writeFile("./test-output.txt", newContent);
  console.log("Overwrite result: " + overwriteResult);
  const updatedRead = fs.readFile("./test-output.txt");
  ...
  const subdirResult = fs.writeFile("./output/test.txt", "Subdirectory test");
  console.log("Subdirectory write result: " + subdirResult);
  const subdirRead = fs.readFile("./output/test.txt");
