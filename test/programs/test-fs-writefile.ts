function main() {
  ...
  const result1 = fs.writeFile("./output.txt", "Hello, world!");
  ...
  const result2 = fs.writeFile("./log.txt", content);
  ...
  const result4 = fs.writeFile("./number.txt", 42);
  console.log("Write number result:", result4);
  
  const result5 = fs.writeFile("./boolean.txt", true);
  console.log("Write boolean result:", result5);
  
  const result6 = fs.writeFile("./null.txt", null);
