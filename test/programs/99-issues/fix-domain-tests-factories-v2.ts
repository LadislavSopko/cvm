function main() {
  ...
  for (const testFile of testFiles) {
    ...
    // Extract the count without regex
    var instanceCount = 0;
    if (introspectionResult.indexOf("refactor:") !== -1) {
      // Find the position of "refactor:"
      var refactorPos = introspectionResult.indexOf("refactor:");
      var numberStart = refactorPos + 9; // length of "refactor:"
      var numberStr = "";
      
      // Extract digits after "refactor:"
      for (var i = numberStart; i < introspectionResult.length; i++) {
        var char = introspectionResult.charAt(i);
        if (char >= '0' && char <= '9') {
          numberStr += char;
        } else {
          break;
      ...
      if (numberStr.length > 0) {
        instanceCount = parseInt(numberStr);
      }
    }
  ...
  if (testResult.indexOf("fail") !== -1) {
    var failCount = "unknown";
    if (testResult.indexOf("fail:") !== -1) {
      // Find the position of "fail:"
      var failPos = testResult.indexOf("fail:");
      var numberStart = failPos + 5; // length of "fail:"
      var numberStr = "";
      
      // Extract digits after "fail:"
      for (var i = numberStart; i < testResult.length; i++) {
        var char = testResult.charAt(i);
        if (char >= '0' && char <= '9') {
          numberStr += char;
        } else {
          break;
